    ...
    <section className="border-t border-white/[0.07] py-28 lg:py-36 bg-[#04040C]">
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-start">
          {/* Left: Text */}
          <div>
            <h2 className="font-condensed text-3xl sm:text-4xl lg:text-5xl font-bold uppercase leading-[1.05] text-[#FFFFFF] tracking-tight mb-10">
              Built under<br />real weight.
            </h2>

            <div className="space-y-6 max-w-lg">
              <p className="text-[#A8A8C0] text-base leading-relaxed font-light">
            ...
            <div className="mt-10 space-y-3">
              {credentials.map((c) => (
                <div key={c} className="flex items-center gap-3">
                  <span className="w-1 h-1 rounded-full bg-[#1755E7]" aria-hidden />
            ...
            <div className="mt-12">
              <a
                href="#apply"
                className="inline-flex items-center gap-3 text-[0.65rem] tracking-[0.2em] uppercase text-[#1755E7] hover:text-[#4070F4] font-medium group"
              >
                Apply for Coaching
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="col-span-2 aspect-[2/3] bg-[#070712] border border-white/[0.07] overflow-hidden relative">
            ...
            {mediaBlocks.map(({ label, span, aspect }) => (
              <div
                key={label}
                className={`${span} ${aspect} bg-[#070712] border border-white/[0.07] flex flex-col items-center justify-center gap-3 hover:border-white/[0.12] transition-colors duration-300 group`}
              >
                <div className="w-10 h-10 border border-white/[0.08] flex items-center justify-center group-hover:border-white/[0.15] transition-colors duration-300">
                  <span className="text-[#2D2D40] text-sm">▶</span>
                </div>
                <p className="text-[0.55rem] tracking-[0.2em] uppercase text-[#2D2D40]">{label}</p>
              </div>
            ))}
          </div>
        </div>
