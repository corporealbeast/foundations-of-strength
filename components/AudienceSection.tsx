...
function AudienceCard({
  ...
  return (
    <div
      className={`${dark ? 'bg-[#070712]' : 'bg-[#000000]'} p-10 lg:p-14 flex flex-col justify-between min-h-[440px] hover:brightness-110 transition-all duration-300 relative overflow-hidden`}
    >
      {/* Left edge accent */}
      <div className="absolute top-0 left-0 w-[2px] h-16 bg-[#1755E7]/40" aria-hidden />

      <div>
        <p className="text-[0.58rem] tracking-[0.22em] uppercase text-[#1755E7] font-medium mb-6">
          {card.label}
        </p>
        <h3 className="font-condensed text-3xl lg:text-4xl font-bold uppercase text-[#FFFFFF] leading-tight mb-6 whitespace-pre-line">
          {card.headline}
        </h3>
      ...
      <div className="mt-10 grid grid-cols-2 gap-3">
        {card.tags.map((tag) => (
          <div key={tag} className="border border-white/[0.07] px-4 py-2.5">
            <p className="text-[0.6rem] tracking-[0.15em] uppercase text-[#5C5C78]">{tag}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        {/* Headline */}
        <h2 className="font-condensed text-3xl sm:text-4xl lg:text-5xl font-bold uppercase leading-[1.05] text-[#FFFFFF] tracking-tight mb-14 max-w-3xl">
